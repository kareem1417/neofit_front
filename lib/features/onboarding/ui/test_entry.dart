...
class _TestEntryScreenState extends State<TestEntryScreen> {
  ...
  void initState() {
    super.initState();
    // ✅ UPDATED: Use fetchLatestSnapshot for editing mode
    if (widget.isEditing) {
      context.read<AuthCubit>().fetchLatestSnapshot(widget.sportId);
    } else {
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Text(
                          widget.isEditing ? 'EDIT TESTS' : 'BASELINE TESTS',
                          style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Colors.white, letterSpacing: 1.0)
                        ),
                    ...
                    child: (state is TestsLoaded && state.tests.isNotEmpty)
                        ? ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              // ✅ ADDED: Get controller and set existing value if it exists
                              final controller = _getController(testData['id']);
                              
                              // ✅ ADDED: Set the existing value if available
                              if (controller.text.isEmpty &&
                                  testData['current_value'] != null) {
                                controller.text = testData['current_value'].toString();
                              }
                  ...
                  Padding(
                    ...
                    child: ElevatedButton(
                      // ✅ UPDATED: Different action based on editing mode
                      onPressed: () {
                        if (widget.isEditing) {
                          cubit.createManualSnapshot(_controllers);
                        } else {
                          cubit.submitAthleteDetails(_controllers);
                        }
                      },
                      ...
                      child: Text(
                        widget.isEditing ? 'Save Changes' : 'Calculate My Scores',
  ...
  Widget _buildTestCard({required IconData icon, required String title, required Widget child}) {
    return Container(
      ...
      child: Column(children: [
        Row(children: [
          Icon(icon, color: const Color(0xFF00E5C1), size: 18), 
  ...
  Widget _buildInputField({required TextEditingController controller, required String hint, required String unit}) {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        suffixIcon: Padding(
          padding: const EdgeInsets.all(16), 
          child: Text(unit, style: const TextStyle(color: Colors.white24, fontWeight: FontWeight.bold))
        ),
        filled: true, 
